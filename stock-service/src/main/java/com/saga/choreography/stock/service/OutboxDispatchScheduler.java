...
import com.saga.choreography.stock.repository.OutboxMessageRepository;
...
public class OutboxDispatchScheduler {
    ...
    public void tick() {
        for (OutboxMessageEntity row : outboxMessageRepository.findTop50ByPublishedAtIsNullOrderByCreatedAtAsc()) {
            try {
                publishRow(row);
            } catch (Exception e) {
